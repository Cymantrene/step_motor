...
/* Конвертация Microstep (1..64) в DriverMicrostepMode (DRV_MS_1..DRV_MS_64) */
...
static void apply_microstep(Menu *m) {
	/* Axis хранит microstep и переводит full_steps в microsteps */
	m->a1->microstep = m->microstep;
	m->a2->microstep = m->microstep;

	/* Драйверы: пины MS1/MS2/MS3 выставляем через driver */
